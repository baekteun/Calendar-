...
class addPlanViewModel {
    ...
    func completeAddPlan(_ controller: addPlanViewController){
        ...
        let plan = PlanModel(date: controller.selectedDay, color: controller.colorSelect.selectedColor!.hexString(), title: title, memo: controller.memoField.text, end: controller.endDate.date.toString, isComplete: false, uuid: uuid)
